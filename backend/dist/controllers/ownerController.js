...
exports.getDashboardStats = async (req, res, next) => {
    try {
        ...
        const plansWithCount = await Promise.all(planStats.map(async (stat) => {
...
exports.getMembers = async (req, res, next) => {
    try {
        ...
        const membersWithPlans = await Promise.all(members.map(async (member) => {
...
exports.updateTrainer = async (req, res, next) => {
    try {
        ...
        await TrainerProfile.findOneAndUpdate({ trainerId: req.params.id }, {
            specialty: specialty ? (Array.isArray(specialty) ? specialty : specialty.split(',').map(s => s.trim())) : [],
            experience: experience || 0,
            bio: bio || '',
            salary: salary || 0
